...
(function(){
  ...
  function simplifyLabUI(){
    const luckPanel=document.getElementById('luckList')?.closest('article');
    if(luckPanel) luckPanel.remove();
    const section=document.getElementById('analytics');
    if(section){
      const h=section.querySelector('.section-head h2');
      const p=section.querySelector('.section-head .section-copy');
      if(h) h.textContent="Who’s actually good right now.";
      if(p) p.textContent='Power Rankings blend scoring, all-play strength, recent form, record and consistency into one season-long team-strength score.';
      section.querySelector('.lab-grid')?.classList.add('lab-power-only');
    }
    const powerPanel=document.getElementById('allPlayList')?.closest('article');
    if(powerPanel){
      const small=powerPanel.querySelector('.panel-title small');
      const title=powerPanel.querySelector('.panel-title h3');
      const explain=powerPanel.querySelector('.panel-explain');
      if(small) small.textContent='POWER RANKINGS';
      if(title) title.textContent='Composite team strength';
      if(explain) explain.innerHTML='Power score: <strong>35% all-play strength</strong>, 30% scoring, 20% recent 3-week form, 10% actual record and 5% consistency. Arrows show movement since last week.';
    }
  }
  ...
  setupRecapPromptUI();
  const ready=setInterval(()=>{if(state?.league&&state?.rosters?.length&&state?.currentWeek){clearInterval(ready);simplifyLabUI();buildSeasonAnalytics().catch(console.error);setupRecapPromptUI();}},80);
